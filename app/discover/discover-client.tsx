...
import { DISCOVER_PAGE_SIZE as DEFAULT_SIZE } from '@/lib/list-config';

interface DiscoverClientProps {
  /** Server-rendered default view: popular, page 1, DEFAULT_SIZE, no query. */
  initialScripts: HubScript[];
  initialTotal: number;
}

export function DiscoverClient({ initialScripts, initialTotal }: DiscoverClientProps) {
  ...
  useEffect(() => {
    // The server already rendered exactly this view, so don't refetch it on
    // mount. Any URL param (?sort/?q/?page/?size) lands here on the next pass
    // with non-default state and does fetch.
    if (isFirstRun.current) {
      isFirstRun.current = false;
      if (sort === 'popular' && !debounced && page === 0 && pageSize === DEFAULT_SIZE) return;
    }
