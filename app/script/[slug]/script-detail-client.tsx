...
export interface ScriptDetail extends HubScript {
  description: string;
  updated_at: string;
  loadstring: string;
  reactions: Reactions;
...
const EMPTY_REACTIONS: Reactions = {
  likes: 0, dislikes: 0, favorites: 0, me: { like: false, dislike: false, favorite: false },
};
...
export function ScriptDetailClient({
  ...
  useEffect(() => {
    ...
    fetch(`/api/scripts/public/${encodeURIComponent(slug)}?anon=${anon}`, { headers: authHeaders() })
              ...
              <Card className="bg-card border-border">
                <CardContent className="p-5">
                  ...
                  <p className="text-sm text-muted-foreground mt-0.5">
                    {script.game && script.game !== 'Universal' ? `${script.game} script` : 'Universal Roblox script'}
                  </p>
