...
interface HomeClientProps {
  /** Server-rendered first page of the catalog (recent, no search). */
  initialScripts: HubScript[];
  initialGames: GameLink[];
  initialDiscover: HubScript[];
}

export function HomeClient({ initialScripts, initialGames, initialDiscover }: HomeClientProps) {
  const [scripts, setScripts] = useState<HubScript[]>(initialScripts);
  ...
  // Games + community picks never change from user interaction, so they stay
  // exactly as the server rendered them.
  const games = initialGames;
  const discover = initialDiscover;
  ...
  useEffect(() => {
    // On mount the server already gave us exactly this list (recent, no search),
    // so skip the duplicate request. A ?search= in the URL re-runs this effect
    // with a non-empty query, which does fetch.
    if (isFirstRun.current) {
      isFirstRun.current = false;
      if (!debounced && sort === 'recent') return;
    }
